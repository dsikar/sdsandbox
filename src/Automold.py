# From https://github.com/dsikar/automold/blob/master/Automold.py
# import glob
import cv2 as cv2
import numpy as np
...
##rain_type='drizzle','heavy','torrential'
def add_rain_single(image,slant=-1,drop_length=20,drop_width=1,drop_color=(200,200,200),rain_type='None'): ## (200,200,200) a shade of gray
    # Add rain for single image array for the predict_client.py script
    slant_extreme=slant
    imshape = image.shape
    if slant_extreme==-1:
        slant= np.random.randint(-10,10) ##generate random slant if no slant value is given
